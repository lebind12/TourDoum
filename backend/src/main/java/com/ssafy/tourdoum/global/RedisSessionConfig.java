 ...
 *
 * <p>ADR-0005: {@code spring.session.store-type=redis} 일 때만 이 설정을 로드한다 (matchIfMissing=true).
 * 테스트 컨텍스트에서 store-type=none 설정 시 @EnableRedisIndexedHttpSession 이 Redis 연결을 시도하지 않도록
 * 방어한다. store-type 프로퍼티가 없으면 기존 동작(Redis 세션 활성화)을 유지한다.
 */
@Configuration
