...
const SEED_ACCOMMODATIONS: Accommodation[] = [
	...
	},
	{
		...
		longitude: 129.2108,
		description:
			"황리단길 바로 옆에 위치한 모텔. 불국사, 석굴암 등 경주 명소 접근이 편리합니다.",
	...
	},
	{
		...
		longitude: 128.4583,
		description:
			"설악산 입구 인근의 아늑한 펜션. 등산 후 피로를 풀기에 최적의 장소입니다.",
...
export const useAccommodationsStore = defineStore("accommodations", () => {
	...
	const types = computed<Accommodation["type"][]>(
		() =>
			[
				...new Set(SEED_ACCOMMODATIONS.map((a) => a.type)),
			] as Accommodation["type"][],
	);

	const sidos = computed(() => [
		...new Set(SEED_ACCOMMODATIONS.map((a) => a.sido)),
	]);
